...
def get_patient_services(patient_id):
    ...
    try:
        cursor.execute("SELECT service_name FROM services WHERE patient_id = ?", (patient_id,))
        services = cursor.fetchall()
        return [service[0] for service in services ] # Extract service names 
    finally:
        conn.close()
...
st.title("British Columbia Home and Community Care App")
st.image("images/beautiful columbians.webp")

tab0, tab1, tab2, tab3, tab4, tab5 = st.tabs(["Overview","Patients Details","Services", "Booking", "Health Authorities Map", "Data Analysis" ])

with tab0:
    st.header("overview of the health scheduler")
    st.video("images/testing video.mp4")
...
with tab2:
    ...
    if st.session_state.patient_id:
        ...
        st.warning("Please log in to select or update your services.")    
...
with tab3:
    if st.session_state.patient_id:
        ...
        st.warning("Please log in to select or update your appointment.")
